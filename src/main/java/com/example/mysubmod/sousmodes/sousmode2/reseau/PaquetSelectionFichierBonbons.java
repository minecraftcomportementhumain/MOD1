        ...
        ctx.get().enqueueWork(() -> {
            ...
            if (joueur != null) {
                ...
                if (GestionnaireSousModes.getInstance().estAdmin(joueur)) {
                    ...
                    GestionnaireSousMode2.getInstance().definirFichierApparitionBonbons(paquet.fichierSelectionne);
                    // Démarrer la sélection d'île après que le fichier soit défini
                    GestionnaireSousMode2.getInstance().demarrerSelectionIle(joueur.getServer());
                }
            }
        });
